 ...
 *                                                                            *
 ******************************************************************************/
void	zbx_jsonpath_substitute_macro(const char *path, int macro_start, char **macro_value)
{
	char	quote = '\0';
	int	i, esc = 0;
	...
	for (i = 0; '\0' != path[i]; i++)
	{
		...
		if ('\"' == quote)	/* we only escape macros enclosed in double quotes */
		{
			char	*macro_value_esc;

			macro_value_esc = zbx_dyn_escape_string(*macro_value, "\\\"");
			zbx_free(*macro_value);
			*macro_value = macro_value_esc;
		}
