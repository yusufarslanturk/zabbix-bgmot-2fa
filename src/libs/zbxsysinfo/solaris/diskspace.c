...
int	VFS_FS_DISCOVERY(AGENT_REQUEST *request, AGENT_RESULT *result)
{
	...
	while (-1 != getmntent(f, &mt))
	{
		...
		zbx_json_addstring(&j, ZBX_SYSYNFO_FSNAME_MACRO_TAG, mt.mnt_mountp, ZBX_JSON_TYPE_STRING);
		zbx_json_addstring(&j, ZBX_SYSYNFO_FSTYPE_MACRO_TAG, mt.mnt_fstype, ZBX_JSON_TYPE_STRING);
		zbx_json_close(&j);
	}
