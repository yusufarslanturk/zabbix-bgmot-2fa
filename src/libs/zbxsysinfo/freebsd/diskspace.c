...
int	VFS_FS_DISCOVERY(AGENT_REQUEST *request, AGENT_RESULT *result)
{
	...
	for (i = 0; i < rc; i++)
	{
		...
		zbx_json_addstring(&j, ZBX_SYSYNFO_FSNAME_MACRO_TAG, mntbuf[i].f_mntonname, ZBX_JSON_TYPE_STRING);
		zbx_json_addstring(&j, ZBX_SYSYNFO_FSTYPE_MACRO_TAG, mntbuf[i].f_fstypename, ZBX_JSON_TYPE_STRING);
		zbx_json_close(&j);
	}
