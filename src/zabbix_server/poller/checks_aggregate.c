		...
		char **error)
{
	...
	zabbix_log(LOG_LEVEL_DEBUG, "In %s() itemkey:'%s'", __function_name, itemkey);
 ...
 *                                                                            *
 ******************************************************************************/
static int	evaluate_aggregate(DC_ITEM *item, AGENT_RESULT *res, int grp_func, zbx_vector_str_t *groups,
		const char *itemkey, int item_func, const char *param)
{
	...
	zabbix_log(LOG_LEVEL_DEBUG, "In %s() grp_func:%d itemkey:'%s' item_func:%d param:'%s'",
			__function_name, grp_func, itemkey, item_func, ZBX_NULL2STR(param));
