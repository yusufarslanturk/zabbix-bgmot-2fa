...
{
	const char	*__function_name = "get_eventlog6_id";
	int		ret = FAIL;
	...
	{
		/* no data in eventlog */
		zabbix_log(LOG_LEVEL_DEBUG, "%s() EvtNext failed:%s", __function_name, strerror_from_system(GetLastError()));
		*id = 0;
		ret = SUCCEED;
		goto out;
	}
...
out:
	...
	zabbix_log(LOG_LEVEL_DEBUG, "End of %s():%s id:" ZBX_FS_UI64, __function_name, zbx_result_string(ret), id);
		...
		zbx_uint64_t *FirstID, zbx_uint64_t *LastID, char **error)
{
	...
	*LastID = 0;
	...
	{
		/* no data in eventlog */
		zabbix_log(LOG_LEVEL_DEBUG, "%s() first EvtNext failed", __function_name);
		*FirstID = 1;
		*LastID = 1;
		*lastlogsize = 0;
		ret = SUCCEED;
	...
	{
		/* no data in eventlog */
		zabbix_log(LOG_LEVEL_DEBUG, "%s() last EvtNext failed", __function_name);
		*LastID = 1;
	}
	else
...
out:
	...
	zabbix_log(LOG_LEVEL_DEBUG, "End of %s():%s last eventid:%lu", __function_name, zbx_result_string(ret), evt_eventid);
