...
2. if you already have Zabbix server installed (WebUI and MySQL DB on the same host) then login into the Zabbix server as `root`, download this script
```
curl -L -o bg-features-install.sh https://github.com/BGmot/zabbix/raw/4.4.4-bg/bg-scripts/bg-features-install.sh
```
