...
abstract class CTriggerGeneral extends CApiService {
	...
	private function prepareInheritedTriggers(array $tpl_triggers, array $hostids = null, array &$ins_triggers = null,
		...
		$chd_triggers_tpl = self::getHostTriggersByTemplateId(array_keys($tpl_hostids_by_triggerid), $hostids);
	 ...
	 * @return array
	 */
	private static function getHostTriggersByTemplateId(array $tpl_triggerids, array $hostids = null) {
		// Preparing list of triggers by templateid.
		$sql = 'SELECT DISTINCT t.triggerid,t.expression,t.description,t.url,t.status,t.priority,t.comments,t.type,'.
				't.recovery_mode,t.recovery_expression,t.correlation_mode,t.correlation_tag,t.manual_close,'.
				't.templateid,i.hostid'.
	...
	private function getHostTriggersByDescription(array $tpl_triggers_by_description) {
		...
		foreach ($tpl_triggers_by_description as $description => $tpl_triggers) {
			...
			$chd_triggers = DBfetchArray(DBselect(
				'SELECT DISTINCT t.triggerid,t.expression,t.description,t.url,t.status,t.priority,t.comments,t.type,'.
					't.recovery_mode,t.recovery_expression,t.correlation_mode,t.correlation_tag,t.manual_close,'.
					'i.hostid,h.host'.
				' FROM triggers t,functions f,items i,hosts h'.
				' WHERE t.triggerid=f.triggerid'.
	...
	private static function checkTriggerRecoveryMode(array $trigger) {
		...
		elseif (array_key_exists('recovery_expression', $trigger) && $trigger['recovery_expression'] !== '') {
			self::exception(ZBX_API_ERROR_PARAMETERS,
	 ...
	 * @param string $trigger['recovery_mode']
	 *
	 * @throws APIException if validation failed.
	 */
	private static function checkTriggerCorrelationMode(array $trigger) {
		if (array_key_exists('correlation_mode', $trigger)
			...
			if (array_key_exists('recovery_mode', $trigger) && $trigger['recovery_mode'] == ZBX_RECOVERY_MODE_NONE) {
		...
		elseif (array_key_exists('correlation_tag', $trigger) && $trigger['correlation_tag'] !== '') {
			self::exception(ZBX_API_ERROR_PARAMETERS,
