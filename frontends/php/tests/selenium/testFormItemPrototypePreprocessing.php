 ...
 * @backup items
 */
class testFormItemPrototypePreprocessing extends CWebTest {
		...
		return [
			...
			[
				[
					'expected' => TEST_GOOD,
					'fields' =>[
						'Name' => 'Item Prometeus LLD macro in parameters 1',
						'Key' => 'wrong-parameters-macro-1'
					],
					'preprocessing' => [
			...
			[
				[
					'expected' => TEST_GOOD,
					'fields' =>[
						'Name' => 'Item Prometeus LLD macro in parameters 2',
						'Key' => 'wrong-parameters-macro-2'
					],
					'preprocessing' => [
			...
			[
				[
					'expected' => TEST_GOOD,
					'fields' =>[
						'Name' => 'Item Prometheus to JSON LLD macro in parameter 1 ',
				...
				[
					'expected' => TEST_GOOD,
					'fields' =>[
						'Name' => 'Item Prometeus to Json LLD macro in parameter 2',
						'Key' => 'json-parameters-macro-2'
					],
					'preprocessing' => [
						['type' => 'Prometheus pattern', 'parameter_1' => '{label_name="{#LABELVALUE}"}']
	 ...
	 */
	public function testFormItemPrototypePreprocessing_CreatePrometheus($data) {
		$this->executeCreate($data);
	}
	...
	private function executeCreate($data) {
		...
		$this->query('button:Create item prototype')->waitUntilPresent()->one()->click();
		...
		switch ($data['expected']) {
			case TEST_GOOD:
				...
				$this->assertEquals('Item prototype added', $message->getTitle());

				// Check result in frontend form.
				$id = CDBHelper::getValue('SELECT itemid FROM items WHERE key_='.zbx_dbstr($data['fields']['Key']));
				$this->page->open('disc_prototypes.php?form=update&parent_discoveryid='.self::DISCOVERY_RULE_ID.'&itemid='.$id);
				$form->selectTab('Preprocessing');
			...
			case TEST_BAD:
				...
				$this->assertEquals('Cannot add item prototype', $message->getTitle());
	...
	public function testFormItemPrototypePreprocessing_CustomOnFail1($data) {
		...
		$this->page->open('disc_prototypes.php?form=update&parent_discoveryid='.self::DISCOVERY_RULE_ID.'&itemid='.$itemid);
		$form->selectTab('Preprocessing');
