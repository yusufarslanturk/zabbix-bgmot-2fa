...
class testPageTriggerUrl extends CWebTest {
	...
	public $url = 'triggers.php?form=update&triggerid=100032';
	...
	/*
	 * Check trigger without url on Overview page.
	 */
	public function testPageTriggerUrl_OverviewPageTriggerWithoutUrl() {
		$name = '1_trigger_Average';

		$this->page->login()->open('overview.php');
		$table = $this->query('class:list-table')->asTable()->one();
		// Get row of trigger.
		$row = $table->findRow('Triggers', $name);

		// Open trigger context menu.
		$row->query('xpath:.//td[contains(@class, "average-bg")]')->one()->click();
		$popup = CPopupMenuElement::find()->waitUntilVisible()->one();
		// Make sure trigger url not present.
		$this->assertFalse($popup->hasItems('URL'));
	}

	/*
	 * Check resolved trigger url on overview page.
	 */
	...
	public function testPageTriggerUrl_EventDetails() {
		// Open evenet details page of trigger.
		$this->page->login()->open('tr_events.php?triggerid=100032&eventid=9000');
	...
	private function checkTriggerUrl($popup_menu = true) {
		if ($popup_menu) {
			...
			$popup = CPopupMenuElement::find()->waitUntilVisible()->one();
			$this->assertTrue($popup->hasTitles(['TRIGGER', 'HISTORY']));
			$popup->fill('URL');
		}
		else {
