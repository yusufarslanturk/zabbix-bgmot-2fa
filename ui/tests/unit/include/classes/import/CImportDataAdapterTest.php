...
class CImportDataAdapterTest extends TestCase {
	...
	public function testGetItems() {
		...
		$this->assertArrayHasKey('export-host', $result);
		...
		foreach ($host_items as $name => $value) {
			$this->assertArrayHasKey($name, $result['export-host'],
				'Item '.$name.' expected for export-host.'
			);

			$this->assertEquals($value, $result['export-host'][$name],
				'Item '.$name.' should match expected format for export-host.'
			);
		}

		$this->assertArrayHasKey('export-template', $result);
		...
		foreach ($template_items as $name => $value) {
			$this->assertArrayHasKey($name, $result['export-template'],
				'Item '.$name.' expected for export-template.'
			);

			$this->assertEquals($value, $result['export-template'][$name],
				'Item '.$name.' should match expected format for export-template.'
			);
		}
	}
	...
	public function testGetDiscoveryRules() {
		$adapter = $this->getAdapter($this->getHostAndTemplateXml());
		$result = $adapter->getDiscoveryRules();
