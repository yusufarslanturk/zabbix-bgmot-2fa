...
if [ "$DB_TYPE" == "MySQL" ]
then
  echo 'describe users' | mysql -u $DB_USERNAME -p${DB_PASSWORD} -h ${DB_HOST} ${DB_NAME} | grep ggl_secret
elif [ "$DB_TYPE" == "PostgreSQL" ]
then
  PGPASSWORD="$DB_PASSWORD" psql -U $DB_USERNAME -h $DB_HOST -d $DB_NAME -c '\d+ users' | grep ggl_secret
fi
if [ "$?" -eq "0" ]
then
  # DB already patched
...
cd ../../app/views/
cp users.twofa.edit.php ${ZABBIX_INSTALL_PATH}/app/views/
cp js/users.twofa.edit.js.php ${ZABBIX_INSTALL_PATH}/app/views/js/
...
cp ${ZABBIX_INSTALL_PATH}/app/controllers/CControllerUserroleCreate.php ${ZABBIX_INSTALL_PATH}/app/controllers/CControllerUserroleCreate.php-`date +%s`.bak
cp CControllerUserroleCreate.php ${ZABBIX_INSTALL_PATH}/app/controllers/
cp ${ZABBIX_INSTALL_PATH}/app/controllers/CControllerUserroleEdit.php ${ZABBIX_INSTALL_PATH}/app/controllers/CControllerUserroleEdit.php-`date +%s`.bak
cp CControllerUserroleEdit.php ${ZABBIX_INSTALL_PATH}/app/controllers/
